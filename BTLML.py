...
for i in range(1,9): #duyệt qua từ 1-8 (i là số components PCA), 
    ...
    X_bar = pca.transform(scaler_x) #ánh xạ cho tập dữ liệu gốc X thành X_bar với số chiều đã giảm
    X_train, X_test, Y_train, Y_test = train_test_split(X_bar, Y, test_size=0.2, shuffle= False)
...
lable_NhietDo = Label(form, text = "Dự đoán bệnh tiểu đường", font=("Arial", 20), fg = "brown").grid(row = 0, column = 1, pady = 10, sticky="e")
...
lable_ChatBeo = Label(group1, text = "BMI:").grid(row = 6, column = 1, pady = 10,sticky="e")
